...
### Correlation with pink salmon

```{r, echo=FALSE, fig.pos="placeHere", fig.cap="Correlation between ln(pink salmon harvest) and predicted male herring in cast nets"}
# PWS hatchery harvest from Stopha:
pws = data.frame("year"=1997:2015, "hatchery_harvest"= c(24686332,
24760828,
37968264,
33040270,
28466847,
...
ggplot(df[df$Gear=="Cast net",], aes(log(hatchery_harvest), pred)) + geom_point() + xlab("Ln (pink salmon hatchery harvest)") + ylab("Predicted male herring in cast nets") + geom_smooth(method="lm") + theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))
